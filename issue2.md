...
If `supabase status` instead fails with `failed to parse config: 'storage' has invalid keys: ...`, the installed CLI is older than `config.toml` expects — see the failure-signature table at the end of this file for the upgrade path (this repo hit exactly this: CLI 2.54.11 installed, config generated by ~2.110.0). If it fails with `No such container: supabase_db_revive` while `docker ps` shows containers with a different name suffix, an earlier `supabase start` ran under a different `project_id` — see the same table.
...
**Why the GRANT block is here even though §3 never mentions it:** this was discovered by actually running step 9 below, not predicted in advance. On the current Supabase Postgres image, tables created by the `postgres` role (which is what runs your migrations) do **not** automatically get PostgREST-reachable privileges — only tables created by `supabase_admin` do. Skip this block and every `curl` in step 9 returns `permission denied`, including for the service-role key, which is a symptom of a missing GRANT, not a broken RLS policy — `service_role` has `BYPASSRLS` at the Postgres level (confirmable via `select rolname, rolbypassrls from pg_roles;`), so if it still gets denied, look here before you go anywhere near the policies.
...
grep -c "^grant "                   $REPO/supabase/migrations/0002_rls.sql
```
...
**If `psql` isn't installed and you have no sudo** (both true in this WSL setup — the CLI is a standalone binary, not an apt package, and there's no passwordless sudo here): every `psql "$DB_URL" -c "..."` below works identically run through the container instead. Find the container name once, then swap the prefix:

```bash
docker ps --filter "label=com.docker.compose.project=revive" --format '{{.Names}}' | grep db
# -> supabase_db_revive
```

Replace `psql "$DB_URL" -c "..."` with `docker exec -e PGPASSWORD=postgres supabase_db_revive psql -U postgres -d postgres -c "..."` throughout this step and step 8/9. Same SQL, same expected output — only the transport changes.
...
**This query must be scoped to the `public` schema** — Supabase's own `auth`/`storage`/`realtime` schemas define plenty of enums of their own (`aal_level`, `factor_status`, `oauth_client_type`, and a dozen more), and an unscoped `pg_type`/`pg_enum` join returns all of them mixed in with yours. Filter through `pg_namespace`:

```bash
psql "$DB_URL" -c "select t.typname, string_agg(e.enumlabel, ',' order by e.enumsortorder) as values from pg_type t join pg_enum e on e.enumtypid = t.oid join pg_namespace n on n.oid = t.typnamespace where n.nspname = 'public' group by t.typname order by t.typname;"
```
...
| `psql: command not found`, and no sudo to install it | psql not installed, standalone-binary CLI setup with no passwordless sudo | Run every query through the container instead — see the `docker exec` box at the top of step 7 — or use Studio at http://127.0.0.1:54323 |
| `Could not find the supabase-go binary required to bootstrap the local database` | The CLI's companion binary is missing. This CLI ships as two files (`supabase` and `supabase-go`) that must sit in the **same directory**, resolved relative to the running binary's own path, not via `$PATH` search | Extract both from the release tarball into one directory, e.g. `~/.local/bin/` (or anywhere writable, ahead of other `supabase` binaries on `$PATH`); `chmod +x` both |
| `failed to parse config: 'storage' has invalid keys: analytics, s3_protocol, vector` (from `supabase status`/`db reset`) | Installed CLI is older than the `config.toml` schema it's reading | Upgrade the CLI — see `gh release list --repo supabase/cli --exclude-pre-releases` for the current stable tag |
| `supabase status`/`db reset` errors `No such container: supabase_db_<project_id>` while `docker ps` shows containers named `supabase_db_<something-else>` | An earlier `supabase start` ran under a different `project_id` (or a different directory name) than what's now in `config.toml`; the CLI won't recognize containers from a mismatched project | `docker rm -f` the orphaned containers for that old project (find them by `docker ps -a --filter "label=com.docker.compose.project=<old-name>"`), then `supabase start` fresh — safe since no migrations exist yet at that point |
